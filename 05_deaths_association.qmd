...
```{r}
tab1_1 <- gbmt_log_descriptive %>% 
  distinct(country, SALID1, group) %>% 
...
tab1_2 <- gbmt_log_descriptive %>% 
  select(-c(ISO2, SALID1, YEAR, deaths, country)) %>% 
...
tab2_2 <- gbmt_log_descriptive %>% 
  select(-c(ISO2, SALID1, YEAR, deaths, group)) %>% 
  tbl_summary(
    by = country,
    percent = "row"
  ) %>% 
...
gbmt_log_merged4 <- gbmt_log_merged %>% 
  filter(Clusters == 4) %>% 
  augment(model_4_binomial_neg, data = ., type.predict = "response")
...
plot_binomialoffset4 <- gbmt_log_merged4 %>% 
  group_by(group, YEAR) %>% 
  summarise(
    predicted_deaths_100k_mean = mean(predicted_deaths_100k),
    deaths_per_100k_mean = mean(deaths_per_100k),
    lower_ci_mean = mean(lower_ci),  # No es metodológicamente correcto
    upper_ci_mean = mean(upper_ci)   # No es metodológicamente correcto
  ) %>% 
  ungroup() %>% 
  ggplot(aes(x = YEAR, 
           y = predicted_deaths_100k_mean,
           color = group)) + 
  geom_point(aes(y = predicted_deaths_100k_mean), alpha = 0.5,
             position = position_jitter(h = 0.2)) +  # Datos observados
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = lower_ci_mean, ymax = upper_ci_mean, fill = group), alpha = 0.2) + 
  labs(title = "Average Deaths per 100,000 inhabitants by Cluster",
       x = NULL, y = NULL, color = "Cluster", fill = "Cluster") +
  innovar::scale_color_innova("jama") +
  innovar::scale_fill_innova("jama") +
  theme_minimal()

plot_binomialoffset4
```
...
ggsave(
  plot_binomialoffset4,
  filename = "02_output/plots/promedio_muertes_group_binomial4.png",
  dpi = 300,
  bg = "white",
  width = 6,
  height = 4.5
...
plot_binomial2offset4 <- gbmt_log_merged5 %>% 
  filter(Clusters == 4) %>% 
  group_by(Clusters, group, YEAR) %>%
  summarise(
    across(
...
ggsave(
  plot_binomial2offset4,
  filename = "02_output/plots/promedio_muertes_group_binomial2_4.png",
  dpi = 300,
  bg = "white",
  width = 7,
  height = 4.5
